...
export default function Navbar() {
  ...
  return (
    <footer>
      <nav 
        className={`${bgClass} px-6 flex items-center justify-between xl:px-24 border-b border-neutral-500 fixed top-0 right-0 left-0 h-24 shadow-xl z-[100]`}
      >
        ...
        <div className="flex items-center justify-center space-x-3 lg:space-x-12">
          {bgClass === 'bg-white text-black' && <p className="text-xs text-orange-400">Yikes!</p>}
          {bgClass === 'bg-black text-white' && <p className="text-xs">Z-z-z!</p>}
          <ThemeChanger />
          <Menu />
        </div>
      </nav>
